...
public abstract class AbstractService<M extends AbstractMapper<T>,T extends IModel<TKey>,TKey extends Serializable> extends BaseService implements IService<T,TKey> {
    ...
    protected Class<T> getModelClass(){
        if(modelClass == null){
            synchronized (this){
                if(modelClass == null){
                    modelClass = (Class<T>) ReflectionKit.getSuperClassGenericType(getClass(), 1);
                }
            }
        }
        return modelClass;
    ...
    protected Class<TKey> getKeyClass(){
        if(keyClass == null){
            synchronized (this){
                if(keyClass == null){
                    keyClass = (Class<TKey>)ReflectionKit.getSuperClassGenericType(getClass(), 1);
                }
            }
        }
        return keyClass;
    ...
    public boolean update(T record) {
        try {
            int result = mapper.updateById(record);
            return result >= 1;
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return false;
